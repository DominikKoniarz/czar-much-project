...
const DeviceEcoCheckbox = ({ deviceMinPower, deviceId }: Props) => {
	...
	const [minPower, setMinPower] = useState<number | null>(deviceMinPower ?? 0);
		...
		<div className="flex items-center gap-2">
			...
			<div className="flex items-center">
				<Input
					...
					step={0.1}
				/>
				<span className={cn("ml-1", !enabled && "opacity-50")}> kW</span>
				{isPending && (
					<div className="w-4.5 h-4.5 ml-1.5">
						<LoaderCircle className="animate-spin h-full w-full text-blue-500" />
					</div>
				)}
			</div>
		</div>
